...
function StatsModal({ history, loading, onClose }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><section className="modal stats-modal" aria-label="Your 30-day statistics" onMouseDown={(event) => event.stopPropagation()}><div className="modal-header"><div><p className="eyebrow">Your progress</p><h2><UserRound size={23} />Last 30 days</h2></div><button className="icon-button" onClick={onClose} title="Close"><X size={19} /></button></div>{loading ? <div className="stats-loading"><span className="loader" />Loading your history</div> : <><p className="stats-summary"><strong>{total}</strong> habit completions across the last 30 days. Today appears first.</p><div className="stats-calendar">{days.map((day) => { const date = new Date(`${day.date}T00:00:00`); const level = day.completed_habits ? Math.min(day.completed_habits, 4) : 0; return <div className={`calendar-day completion-level-${level}`} key={day.date} title={`${dateFormat.format(date)}: ${day.completed_habits} habits completed`}><span>{monthFormat.format(date)}</span><b>{date.getDate()}</b><em>{day.completed_habits}</em></div> })}</div><p className="calendar-key" aria-label="Completion intensity: no activity, one, two, three, and four or more habits"><span><i className="completion-level-0" />0</span><span><i className="completion-level-1" />1</span><span><i className="completion-level-2" />2</span><span><i className="completion-level-3" />3</span><span><i className="completion-level-4" />4+</span></p></>}</section></div>
}
